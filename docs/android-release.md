...
The separate publish job downloads that exact artifact and verifies its checksum before receiving any R2 credentials. It is skipped unless all three repository variables below explicitly authorize the inner-test channel, and it runs in the `android-preprod-publish` environment. It creates the commit-addressed object at `android/preprod/builds/<commit SHA>/windnote.apk` only when absent; a rerun never overwrites it and must prove its downloaded bytes, checksum, and preproduction package metadata match. Objects created between the package-identity cutover and package-metadata rollout are accepted only when their digest matches and GitHub proves their commit is at or after the reviewed cutover commit; older production-identity objects remain ineligible. Before promotion it saves the current stable object, arms rollback before mutating `latest`, copies the verified object to `android/preprod/latest/windnote.apk`, and verifies the R2 versioned object, R2 stable object, and public download byte-for-byte. Any post-promotion failure restores the previous stable object (or removes the first unverified stable object), then removes the temporary backup.

`https://pub-9d36120697ca455b9fb0e430da8b9481.r2.dev/android/preprod/latest/windnote.apk`

This URL is public and unauthenticated: anyone who knows it can download the preproduction APK. Cloudflare documents `r2.dev` as a development URL, so it is suitable only for the current inner test; use a custom R2 domain before broad production traffic. This preproduction path is intentionally separate from the formal `android/latest/windnote.apk` channel. It does not create a GitHub Release, does not enable `ANDROID_PUBLIC_RELEASE_ENABLED`, and is not evidence that the formal legal distribution gate below passed.
...
- R2 credentials used by that environment/job.
...
The rollback refuses mutable aliases and non-preproduction package metadata. It rewrites `latest` with the stable five-minute cache policy, verifies both R2 and public bytes, and restores the pre-rollback object if verification fails.
