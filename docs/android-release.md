...
The one-time R2 cutover has a mandatory rollback checkpoint. Before merging or enabling the COS publisher, seed the private immutable object `android/preprod/builds/7cb7cb9f4e6f29fb5d6d3f6cb326d5d1338403a7/windnote.apk` from Actions run `33302647501` (`android-preprod-v1.0.1`) and verify SHA-256 `4e31cb02949f838e372e469f2b9ad28bba11fc58b32b305bf0f56f1435237253`, package metadata `com.yiboding.circleim.preprod`, APK headers, and immutable cache metadata after upload. That commit is the only successful preproduction-identity build before the COS cutover: the identity-cutover run failed, while earlier successful objects use the retired production package identity and are intentionally ineligible. Keep the R2 latest object available until the first COS publish, rollback drill, custom-domain download, and both website locales are verified.
