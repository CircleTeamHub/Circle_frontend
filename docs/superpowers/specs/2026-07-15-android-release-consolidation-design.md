...
3. `publish` depends on `build`, is disabled unless `ANDROID_PUBLIC_RELEASE_ENABLED=true`, and targets the `android-release-publish` GitHub environment. That environment must have required reviewers and contain the distribution approval evidence variables plus the cross-repository `RELEASES_TOKEN`. The job downloads the exact private artifact, validates the approval variables, and invokes the existing immutable publisher. The token is exposed only to the publishing step. This path is intentionally unavailable on the repository's current GitHub Free/private configuration: GitHub does not provide private-repository environment secrets or required reviewers on that plan. It must remain disabled until the organization gains the necessary GitHub capability or deliberately changes repository visibility.
...
- The current verified state is `CircleTeamHub` plan `free` with `Circle_frontend` private, so that verification cannot pass. Documentation must not instruct operators to enable public publishing under this state or suggest bypassing the legal gate with a repository-scoped token.
...
After verification, the implementation commits are pushed to `codex/android-release-automation`, updating PR #57. Its PR description is replaced with the current fail-closed limitation, future setup instructions for the protected environment, required secrets/variables, evidence gate, and validation performed. The instructions include commands to verify the organization plan, required-reviewer protection, and absence of a repository-scoped `RELEASES_TOKEN`; they permit setting `ANDROID_PUBLIC_RELEASE_ENABLED=true` only after every prerequisite passes. PR #56 is then closed as superseded with a short link to PR #57; no force-push or history rewrite is used.
