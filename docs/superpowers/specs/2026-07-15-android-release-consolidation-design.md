...
After verification, the implementation commits are pushed to `codex/android-release-automation`, updating PR #57. Its PR description is replaced with setup instructions for the protected environment, required secrets/variables, evidence gate, and validation performed. PR #56 is then closed as superseded with a short link to PR #57; no force-push or history rewrite is used.
