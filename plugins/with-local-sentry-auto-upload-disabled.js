...
const DISABLE_AUTO_UPLOAD_BLOCK = `if [[ "$CONFIGURATION" = *Debug* && ( -z "$SENTRY_AUTH_TOKEN" || -z "$SENTRY_ORG" || -z "$SENTRY_PROJECT" ) ]]; then
...
tasks.configureEach { task ->
    if (task.name.contains("SentryUpload")) {
        task.onlyIf {
            ...
            def isCiBuild = System.getenv("CI") == "true" || System.getenv("EAS_BUILD") == "true"
            return hasUploadConfig || isCiBuild
        }
    }
}
`;
