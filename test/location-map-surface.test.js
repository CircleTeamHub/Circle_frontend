...
test('native map surface keeps the WebView bridge and its hardening', () => {
  ...
  // #172 的收口随载体抽取一起搬进来：地图页锁死在专用假 origin 里，
  // 页内任何跳转都出不去；不给 DOM storage、不开新窗口。
  assert.match(native, /appassets\.invalid/);
  assert.match(native, /originWhitelist=\{\[`\$\{MAP_ORIGIN\}\/\*`\]\}/);
  assert.match(native, /domStorageEnabled=\{false\}/);
  assert.match(native, /setSupportMultipleWindows=\{false\}/);
