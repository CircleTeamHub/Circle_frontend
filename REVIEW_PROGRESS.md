...
- **Pending decisions:** 39 (was 38; +#66 CreatePost remaining i18n grind, +#67 EditFriendTags partial-failure; #65 mock-user data closed in-batch)
...
|---|---|---|---|---|
| 🔁 | `src/stores/authStore.ts` | 148→144 | 1 / 3 / 2 | Patched: setUser bug + version:1 + tighter rehydrate validation. Remaining: token storage → SecureStore decision. [reviews/01-auth.md](reviews/01-auth.md) |
...
| 🔁 | `src/services/auth/session.ts` | 49→81 | 0 / 4 / 2 | Patched: dev-log on swallowed errors + MMKV fallback removal + unregister fn + dedup + auth-first reorder. Tests rewritten and passing. [reviews/01-auth.md](reviews/01-auth.md) |
...
|---|---|---|---|---|---|
| 1 | 🔴 HIGH | 01 | `src/stores/authStore.ts:74-148` | Tokens persisted in MMKV (unencrypted on Android by default) instead of SecureStore | **A.** Switch tokens to `expo-secure-store` (cleanest). **B.** Encrypt MMKV with key in SecureStore. **C.** Accept current risk, document in MODULE_OVERVIEW. |
...
1. **#1 (token storage)** — security posture, single answer, can be patched in <30 min once picked.
2. **#10 (chat-data privacy)** — privacy/compliance call, will shape #3 (which other stores to clear) and the eventual `wipeUserData()` primitive.
3. **#4 (deviceName PII)** — fast decision, fast patch.
4. **#5 (changePassword teardown)** — UX-affecting, decide before users start changing passwords in prod.
5. **#3** — depends on #10 outcome; do them together.
6. **#11 (retry util)** — small but blocks other batches that will want the same primitive (chat send, history pagination, upload).
7. **#2 (auth retry sentinel)** — bundle into a future auth-error-boundary pass.
8. **#6 (error code map)** — blocked on backend; ping backend team for code list.
9. **#7, #8, #9** — non-urgent maintenance / safety hygiene.
